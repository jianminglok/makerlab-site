...
class PinnedController extends Controller
{
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    public function commentPinned(Request $request, ResponseContract $response, FeedModel $feed, CommentModel $comment)
    {
        $user = $request->user();
        if ($comment->user_id !== $user->id) {
            return $response->json(['message' => ['你没有权限申请']])->setStatusCode(403);
        } elseif ($feed->pinnedComments()->newPivotStatementForId($comment->id)->first()) {
