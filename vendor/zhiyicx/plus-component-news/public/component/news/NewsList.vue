        ...
        <table class="table table-hover text-center table-striped table-bordered">
          ...
          <tbody>
            <tr v-if='newsList.length' v-for="news in newsList" :key="news.id" :class="status2style(news.audit_status)">
              ...
              <td width='10%'>{{ news.user.name }}</td>
              <td width='10%'>
                <a v-if='news.storage' :href='"/api/v2/files/" + news.storage.id' target="_blank">
                  <img :src='"/api/v2/files/" + news.storage.id' class="small_img" :alt="news.storage.id">
                </a>
  ...
  methods: {
    ...
    getNewsList(page) {
      ...
      request.get('/news/admin/news', {
        ...
        this.loadding = false;
        if (!response.data.status) return this.tips.error = '获取资讯列表失败！';
        let {
          last_page = 0,
            data = [],
            current_page = 1,
    ...
    deleteNews(_id) {
      ...
      request.delete(`/news/admin/news/del/${_id}/news`)
        .then(response => {
          this.deleteID = null;
          if (!response.data.status) return this.tips.error = '删除资讯失败！';
          this.getNewsList();
        }).catch(({
          response: {
            data: {
    ...
    checkNews(news) {
      if (news && news.id && news.audit_status === 1) {
        request.post(`/news/admin/news/audit/${news.id}`, {
          ...
          if (!response.data.status) return this.tips.error = '审核操作失败！';
          this.getNewsList();
        }).catch(({
          response: {
            data: {
    ...
    backNews(news) {
      if (news && news.id && news.audit_status === 1) {
        request.post(`/news/admin/news/audit/${news.id}`, {
          ...
          if (!response.data.status) return this.tips.error = '驳回操作失败！';
          this.getNewsList();
        }).catch(({
          response: {
            data: {
